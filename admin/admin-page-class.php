...
if ( ! class_exists( 'TOMLIVECHAT_Admin_Page_Class') ) :
 ...
 */
  ...
  class TOMLIVECHAT_Admin_Page_Class {
    ...
    public function DisplayPage() {
      ...
      wp_nonce_field( basename(__FILE__), 'TOMLIVECHAT_Admin_Page_Class_nonce' );
  ...
  public function insert_images() {
    ...
    $html = '';
    $post_attachments = sanitize_text_field($_POST['attachments']);
    foreach( $post_attachments as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image','apc' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete' ,'apc') . "' /></a>";
  ...
  */
  public function wp_ajax_delete_image() {
      $field_id_formated = sanitize_text_field($_GET['field_id']);
    $field_id = isset( $field_id_formated ) ? $field_id_formated : 0;
  ...
  public function reorder_images() {
    ...
    list( $order, $post_id, $key, $nonce ) = explode( '|', $post_data );
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function save($repeater = false) {
    ...
    //check nonce
    if ( ! check_admin_referer( basename( __FILE__ ), 'TOMLIVECHAT_Admin_Page_Class_nonce') )
      return;
    ...
    foreach ( $this->_fields as $field ) {
      if(!in_array($field['type'],$skip)){
        ...
        $new = ( isset( $post_name ) ) ? $post_name : ( ( isset($field['multiple']) && $field['multiple']) ? array() : '' );


        //Validate and senitize meta value
        //issue #27
        $validationClass = apply_filters('apc_validattion_class_name', 'TOMLIVECHAT_Admin_Page_Class_Validate',$this);
    ...
    <div class="apc_ie_panel field">
      <div style="padding 10px;" class="apc_export"><h3>'.__('Export','apc').'</h3>
        ...
        <div class="export_code">
          ...
          <div class="export_status" style="display: none;"></div>
      ...
      <div style="padding 10px;" class="apc_import"><h3>'.__('Import','apc').'</h3>
        ...
        <div class="import_code">
          ...
          <div class="import_status" style="display: none;"></div>
